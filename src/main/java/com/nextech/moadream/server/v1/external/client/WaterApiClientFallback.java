...
@Slf4j
@Component
public class WaterApiClientFallback implements WaterApiClient {

    @Override
    public UtilityUsageResponse getUsageData(UtilityUsageRequest request) {
        log.error("Water API fallback triggered for customer: {}", request.getCustomerId());
        return UtilityUsageResponse.builder().customerId(request.getCustomerId()).utilityType(request.getUtilityType())
                .records(Collections.emptyList()).status("FAILED").message("수도 사용량 조회 서비스가 일시적으로 사용 불가능합니다.").build();
    }
}
